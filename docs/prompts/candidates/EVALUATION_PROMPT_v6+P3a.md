...
When a PRIOR-CALL LEDGER is supplied in the user message, apply the
question "did they give guidance and miss it, on something they had data
to forecast?" on the basis of the ledger. Consult the PRIOR-CALL LEDGER
first. Weigh `miss_pct` against how tightly the company itself guided
(`range_width_pct`) — a miss small relative to the company's own stated
precision may not be a stumble; say which and why. A promise graded
...
ARGUMENT TEST already tells you how to weigh a management claim against its
own track record.
...
## LEDGER READING
Only when a PRIOR-CALL LEDGER is supplied (otherwise write "No ledger."):
1. Which ledger promise is the one the thesis rests on?
2. Was it met?
3. If missed, does any beat elsewhere repair the specific mechanism that
   failed — or is it a different thing?
A beat on one line does not lend credibility to a miss on another.
...
---STRUCTURED---
{
  ...
  "ledgerLoadBearingMetric": null,
  "ledgerLoadBearingOutcome": "none",
  "ledgerMissCount": 0,
  "ledgerMaxMissPct": null,
  "ledgerRevisedDownCount": 0,
  "ledgerWithdrawnCount": 0
}
---END STRUCTURED---
...
- mitigationCapabilityTrackRecord: "strong" | "mixed" |
  "weak" | "unproven" | null
- ledgerLoadBearingMetric: string | null (the ledger promise the thesis rests on; null if no ledger)
- ledgerLoadBearingOutcome: "met" | "partial" | "missed" | "none"
- ledgerMissCount: integer (ledger promises graded missed)
- ledgerMaxMissPct: number | null (largest miss_pct in the ledger)
- ledgerRevisedDownCount: integer (full-year promises revised downward)
- ledgerWithdrawnCount: integer (ledger promises graded withdrawn)
