# READ classifier v3 (measuring tool, not a candidate)
# Used by prompts/winners-missed-v3.md. v2 with the `discounted` field removed
# (its definition matched B's standard phrasing and failed v2's pre-flight);
# nothing else changed. Sees only an analyst's short read of an earnings call.
# No registry entry.
