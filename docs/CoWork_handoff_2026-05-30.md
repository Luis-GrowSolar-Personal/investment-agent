...
- **Crypto exchange accounts (e.g. Kraken)** — add support for non-Schwab accounts held at crypto exchanges. Key characteristics: always taxable; uses exchange-specific API (not Schwab); may support staking (treated similarly to dividend reinvestment — adds to position over time; taxable event in the year received per IRS rules); position types are crypto only. Account type would be a new value: `"crypto_exchange"`. Display as "Crypto Exchange" badge alongside the owner badge.

- **Reinvested dividends** — securities like AAPL, NVDA, ORCL pay cash dividends that can be automatically reinvested as fractional shares. These create new tax lots with a cost basis equal to the dividend amount, and the acquired date is the reinvestment date. The transaction JSON already includes `Reinvest Shares` and `Qual Div Reinvest` actions (currently partially handled). Full support needed: (1) correctly reconstruct fractional lots from reinvestment transactions, (2) display a "DRIP" source badge on those lots (distinct from MANUAL and IMPORT), (3) account for them in cost basis calculations.
