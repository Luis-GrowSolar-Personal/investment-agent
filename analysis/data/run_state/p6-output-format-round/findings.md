...
- INCIDENT (cost): a duplicate arm-C+noise batch (msgbatch_01XBKE7mSMWe9S8xiwArCd46, 1,237 requests, ~$51 if it had run to completion) was created by a first submit-c whose id did not persist. Found by listing batches; cancelled. Recorded C batch: msgbatch_01BVc5EtKqYEVtv6K9YZwmJP. Actual billed amount for the orphan must be read from the console; the wrap-up reports it as unknown-until-checked. Cause not fully established (progress.json was overwritten or not saved between the two processes); a spend-counter double-count bug was found and fixed in the same episode.
