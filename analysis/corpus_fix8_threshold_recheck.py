...
OUT = REPO / "analysis/data/corpus_v2/STEP_C_AT_FIX8_CORRECTED_COUNT.json"
N_LIST = [107]
...
def main():
    ...
    out = {
        ...
        "pass": "corpus-fix-8, Step E -- threshold at the expanded corpus's corrected count",
        "n": 107,
        "n_note": "107 = train+tune tickers in SPLIT_V5_EXPANSION.json (108), minus WOLF "
                  "(S4, unrecoverable price gap, carve-out excludes it from the count). "
                  "159 companies total, 156 gradable, 49 gradable in the 51-company holdout "
                  "(NOVA and SPWR excluded from that count on the same carve-out basis).",
        "sqrt_estimates": {
            "published_baseline_24_over_sqrt_n": round(24 / math.sqrt(107), 2),
            "this_run_recheck_20_over_sqrt_n": round(20 / math.sqrt(107), 2),
        },
