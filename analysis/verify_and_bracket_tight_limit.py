...
DRIVER_FILE = "analysis/verify_and_bracket_tight_limit.py"
...
def verify_2point5_cell(events_full, prices, type_fn, driver_fn, tier_fn,
                         spy, qqq, tmfc, ew_final):
    """Step 1: verify the 2.5pp cell BEFORE any bracketing sweep runs.
    ...
    for mode in ("no_reserve", "swap_funding"):
        lines.append(f"\n--- {mode}, 2.5pp, forward draw ---")
        r = run_cell(events_full, prices, type_fn, driver_fn, tier_fn,
                     funding_mode=mode, session_change_limit_pp=2.5)

        # 1a: independent drawdown recomputation
        indep_dd, peak_i, trough_i = independent_max_drawdown(r["daily_snapshots"])
        reported_dd_dollars_peak = r["daily_snapshots"][peak_i].total_value
        reported_dd_dollars_trough = r["daily_snapshots"][trough_i].total_value
        dd_diff_pp = abs(indep_dd - r["max_dd"]) * 100
        check("1a independent drawdown recompute",
              dd_diff_pp < 0.01,
              f"compute_summary={r['max_dd']*100:.4f}%  independent={indep_dd*100:.4f}%  "
              f"diff={dd_diff_pp:.4f}pp  (peak {r['daily_snapshots'][peak_i].date} "
              f"${reported_dd_dollars_peak:,.2f} -> trough {r['daily_snapshots'][trough_i].date} "
              f"${reported_dd_dollars_trough:,.2f})")

        # 1b invariant #2: max position weight vs tier cap, by ticker
        max_weight_by_ticker = {}
        for snap in r["daily_snapshots"]:
            if snap.total_value <= 0:
                continue
            for t, v in snap.position_values.items():
                w = v / snap.total_value * 100
                if w > max_weight_by_ticker.get(t, 0):
                    max_weight_by_ticker[t] = w
        cap_by_ticker = {}
        for t in ALL16:
            tc = type_fn(t)
            tier = tier_fn(t)
            cap_by_ticker[t] = _type_cap(tc, tier, None)
        breaches = [(t, w, cap_by_ticker.get(t)) for t, w in max_weight_by_ticker.items()
                    if cap_by_ticker.get(t) is not None and w > cap_by_ticker[t] + 0.5]
        worst = max(max_weight_by_ticker.items(), key=lambda kv: kv[1] - cap_by_ticker.get(kv[0], 0),
                    default=(None, 0))
        check("1b invariant #2 (no position exceeds tier cap)",
              len(breaches) == 0,
              f"breaches={breaches if breaches else 'none'}; max observed weight: "
              f"{worst[0]}={worst[1]:.1f}% (cap {cap_by_ticker.get(worst[0]):.0f}%)"
              if worst[0] else "no positions held")

        # 1b invariant #9: max single-session position change vs the 2.5pp limit
        pp_changes = [f["pp_change"] for f in r["funding_log"]]
        max_pp_change = max(pp_changes) if pp_changes else 0.0
        check("1b invariant #9 (session move <= limit + rounding)",
              max_pp_change <= 2.5 + 0.5,
              f"max single-session Add-side position change observed: {max_pp_change:.2f}pp "
              f"against a 2.5pp limit (Trim/Exit-side moves not included in this measurement "
              f"-- those are bounded by pre-existing v2/v3 trim/exit rules, not this limit)")

        # 1b invariant #5: no InsufficientCash / skipped_events
        skipped = r["skipped_events"]
        check("1b invariant #5 (no pre-trade-state trade-set sizing defect observed)",
              len(skipped) == 0,
              f"skipped_events: {len(skipped)}" + (f" -- {skipped[:5]}" if skipped else ""))

        # 1c: where does the drawdown reduction come from
        cash_ratios = [s.cash_total / s.total_value for s in r["daily_snapshots"] if s.total_value > 0]
        avg_cash = statistics.mean(cash_ratios) * 100
        med_cash = statistics.median(cash_ratios) * 100
        trough_snap = r["daily_snapshots"][trough_i]
        ...
        lines.append(f"  1c cash profile: avg cash={avg_cash:.1f}%  median cash={med_cash:.1f}%")
        lines.append(f"      at drawdown trough ({trough_snap.date}): invested={trough_invested_pct:.1f}%  "
                     f"cash={trough_cash_pct:.1f}%")
        lines.append(f"      trough holdings: {sorted(trough_holdings.items(), key=lambda kv: -kv[1])}")
        final_snap = r["daily_snapshots"][-1]
        final_holdings = {t: (v, v / final_snap.total_value * 100) for t, v in final_snap.position_values.items()
                           if v > 1.0}
        lines.append(f"      terminal composition ({final_snap.date}): " +
                     ", ".join(f"{t}={pct:.1f}%" for t, (v, pct) in
                               sorted(final_holdings.items(), key=lambda kv: -kv[1][1])))

        # 1d: binding constraint classification
        binding_counts = {"target gap": 0, "cash available": 0, "session limit": 0}
        for f in r["funding_log"]:
            binding_counts[f["binding"]] = binding_counts.get(f["binding"], 0) + 1
        lines.append(f"  1d binding constraint counts: {binding_counts}")
        dominant = max(binding_counts, key=binding_counts.get) if binding_counts else None
        expected_dominant = "session limit" if mode else None
        check("1d dominant binding constraint is 'session limit' at 2.5pp",
              dominant == "session limit",
              f"counts={binding_counts}, dominant={dominant!r}")

        # 1e: sanity floor
        beats_all = (r["final_value"] > spy and r["final_value"] > qqq and
                     r["final_value"] > tmfc and r["final_value"] > ew_final)
        check("1e beats SPY, QQQ, TMFC, equal-weight",
              beats_all,
              f"2.5pp final=${r['final_value']:,.2f}  SPY=${spy:,.2f}  QQQ=${qqq:,.2f}  "
              f"TMFC=${tmfc:,.2f}  EW=${ew_final:,.2f}")
...
def main() -> int:
    ...
    write_manifest("drawdown-baselines-v2b", events_full, in_window,
    ...
    # ================= STEP 1: VERIFY 2.5pp BEFORE SWEEPING =================
    print("=" * 70)
    print("STEP 1: verifying the 2.5pp cell before any bracketing sweep runs")
    print("=" * 70)
    verified, verify_lines = verify_2point5_cell(
        events_full, prices, type_fn, driver_fn, tier_fn, spy, qqq, tmfc, ew_final)
    for line in verify_lines:
        print(line)
    print()

    write_manifest("step1-verification-2.5pp", events_full, in_window,
                    {"window": [str(START), str(C)], "universe": ALL16},
                    {"all_checks_passed": verified, "detail": verify_lines})

    if not verified:
        print("STOP: Step 1 verification FAILED. Not proceeding to Step 2's "
              "bracketing sweep, per instruction.")
        return 1
    print("Step 1 verification PASSED on both modes. Proceeding to Step 2.\n")

    # ---- Step 2: bracket the low end -- 10 limits x 2 modes x 15 draws = 300 runs ----
    ...
    print(f"=== Step 2: bracketing grid, {len(limit_values)} limits x {len(modes)} modes x "
    ...
    prior_medians = {
        ("no_reserve", "off"): 139355, ("no_reserve", "10"): 179605,
        ("swap_funding", "off"): 155878, ("swap_funding", "10"): 156231,
    }
    anchors_match = True
    for (mode, label), prior_med in prior_medians.items():
        ...
        if not ok:
            anchors_match = False
        print(f"  {mode:14} {label:>4}pp: prior median ${prior_med:,.0f}  this run ${cur_med:,.0f}  "
              f"diff ${diff:,.2f}  -> {'MATCH' if ok else 'MISMATCH'}")
