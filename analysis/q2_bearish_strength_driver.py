...
from analyst_direct_scorer import (
    PriceCache, PRICE_CACHE_PATH, parse_structured, direction_from_score,
...
def bearish_denominators(rows: list[JoinedRow]) -> dict:
    ...
    for split in ("train", "tune"):
        subset = [r for r in rows if r.split == split and r.predicted == "bearish"]
        ...
        base_rate_hits = sum(1 for r in scoreable if r.ground_truth == "bearish")
        base_rate_pct = round(base_rate_hits / n * 100, 1) if n else None
        ungradable = [r for r in subset if r.ticker in UNGRADABLE_TICKERS]
        out[split] = {
            "n_bearish_predicted": len(subset),
            "n_scoreable": n,
            "n_right": hits,
            "pct_right": pct_right,
            "base_rate_pct_bearish_actual": base_rate_pct,
            "n_ungradable_ticker_calls": len(ungradable),
            "ungradable_by_ticker": {
                t: sum(1 for r in subset if r.ticker == t)
                for t in UNGRADABLE_TICKERS
            },
        }
    pooled_scoreable = [
        r for r in rows if r.predicted == "bearish" and r.hit is not None
    ]
    pooled_n = len(pooled_scoreable)
    pooled_base = sum(1 for r in pooled_scoreable if r.ground_truth == "bearish")
    out["pooled"] = {
        "n_scoreable": pooled_n,
        "base_rate_pct_bearish_actual": round(pooled_base / pooled_n * 100, 1) if pooled_n else None,
    }
    return out
