#!/usr/bin/env python3
"""
baseline_v6_tune_batch_driver.py -- prompts/baseline-v6-tune-batch.md

Fetches tune-split transcripts (reusing analysis/ec_fidelity_benchmark_1's
proven vendor client, read-only import per that prompt's instruction), then
scores v6 via the Anthropic Batch API. Twin of baseline_v6_train_batch_driver.py
-- same machinery, tune split instead of train, own state dir and eval cache
so the two never mix. See the prompt for full protocol.

Usage:
  python3 baseline_v6_tune_batch_driver.py events    # resolve symbols + events for all tune tickers
  python3 baseline_v6_tune_batch_driver.py plan       # reconcile against cached raw/, build fetch plan
  python3 baseline_v6_tune_batch_driver.py fetch      # fetch missing transcripts (throttled)
  python3 baseline_v6_tune_batch_driver.py validate3  # print 3 transcripts for eyeball validation
  python3 baseline_v6_tune_batch_driver.py pilot5     # synchronous 5-call pilot
  python3 baseline_v6_tune_batch_driver.py submit     # submit the batch (all remaining requests)
  python3 baseline_v6_tune_batch_driver.py poll       # poll batch status, collect results as they land
  python3 baseline_v6_tune_batch_driver.py writescores  # write results into version-stamped eval cache
"""
...
def load_tune_tickers():
    ...
    tune = split["tune"]
    return [(t, amap.get(t, t)) for t in tune]
...
def cmd_writescores():
    scores_path = STATE_DIR / "scores.jsonl"
    eval_dir = REPO_ROOT / "analysis" / "data" / "evals" / f"v6_{MODEL}_tune"
