...
DRIVER_FILE = "analysis/bracket_tight_limit_corrected.py"
...
def run_five_gates(events_full, prices, type_fn, driver_fn, tier_fn, control_final):
    """Step 1: the five gates, corrected. Only these five stop the run.
    Run on no_reserve/swap_funding at the tightest (0.5pp) and loosest
    (10pp) sampled limits, plus the unmodified control (off).
    ...
    check("Gate 1: standing assertion (no_reserve control = $141,837)",
          abs(control_final - EXPECTED_NO_RESERVE) < 1.0,
          f"${control_final:,.2f}")

    configs = [("no_reserve", None, "off"), ("swap_funding", None, "off"),
               ("no_reserve", 0.5, "0.5pp"), ("swap_funding", 0.5, "0.5pp"),
               ("no_reserve", 10.0, "10pp"), ("swap_funding", 10.0, "10pp")]
    ...
    for mode, limit_pp, label in configs:
        ...
        check(f"Gate 2: invariant #2 at decision time (target_pct <= cap_pct)",
              ...
              f"max(target_pct - cap_pct) observed = "
              f"{max_excess:.4f}pp" if max_excess is not None else "no Add-shaped decisions" +
        ...
        check("Gate 4: invariant #5 (no skipped_events / InsufficientCash)",
              len(skipped) == 0,
...
def main() -> int:
    ...
    write_manifest("drawdown-baselines-v3", events_full, in_window,
    ...
    print("=" * 70)
    print("STEP 1: the five gates (corrected) -- off, 0.5pp, 10pp, both modes")
    print("=" * 70)
    ...
    prior_medians = {
        ("no_reserve", "off"): 139354.62876414592, ("no_reserve", "10"): 179605.3269444481,
    ...
    # ---- "Follow the peak": trough analysis on whichever config has the
    # single highest median across the whole grid ----
    print("\n=== Peak trough analysis (whichever config has the grid's highest median) ===")
    peak_key = max(all_results, key=lambda k: all_results[k]["final_median"])
    peak_res = all_results[peak_key]
    peak_fwd = peak_res["cellinfo"]["forward"]
    print(f"  Grid-wide peak: {peak_key[0]} {peak_key[1]}pp, median ${peak_res['final_median']:,.0f}")
    ...
    final_holdings = {t: v / final_snap.total_value * 100 for t, v in final_snap.position_values.items()
                       if v > 1.0}
    print(f"  Drawdown trough: {trough_snap.date}  invested={trough_invested_pct:.1f}%  cash={trough_cash_pct:.1f}%")
    ...
    write_manifest("peak-trough-analysis", events_full, in_window,
                    {"peak_config": f"{peak_key[0]}-{peak_key[1]}pp"},
                    {"trough_date": str(trough_snap.date), "trough_invested_pct": trough_invested_pct,
                     "trough_cash_pct": trough_cash_pct, "trough_holdings": trough_holdings,
                     "terminal_composition": final_holdings})
